...
first_rolls <- rep(list(dice_placeholder), reps)
...
strategy1 <- yahtzee_rolls %>% 
  ...
  mutate(second_roll_dice = rep(list(dice_placeholder), reps))
...
third_roll <- rep(list(dice_placeholder), reps)
for(i in seq_len(reps))
...
of_a_kind <- vector(mode = "integer", length = reps)
for(i in seq_len(reps)){
